...
topoName = "random_reconnected_s20_h10"
...
###### Topology definition ######
# add switches
agraph = random_topo.get_mid_reconnected_graph(10) # arg is seed
...
edges.append(["router10","pc1"])
edges.append(["router16","pc2"])
edges.append(["router8","pc3"])
edges.append(["router8","pc4"])
edges.append(["router14","pc5"])
edges.append(["router14","pc6"])
edges.append(["router20","pc7"])
edges.append(["router19","pc8"])
edges.append(["router18","pc9"])
edges.append(["router17","pc10"])

# make sure that no switches or hosts have the wrong format and add them to the topo
switches = set()
pc_sources = [1,2,7,9] # set pcs as sources
for e in edges:
  if "pc" not in e[0]:
    switches.add(e[0])
  else:
