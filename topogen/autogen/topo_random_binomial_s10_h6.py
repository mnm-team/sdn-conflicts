...
topoName = "random_binomial_s10_h6_23"
...
eplb.config.targetSwitches.add("router9")
...
appAssetsConfig3.minorAssets.assetItems = ["pc1","pc2"]

# host shadower
hs = model.testbed.apps.add("hs")
hs.config.cookie = "0x440"
hs.config.targetSwitches.add("router1")
hs.config.targetSwitches.add("router5")
...
appAssetsConfig3.minorAssets.assetValue = "pc3"
...
###### Topology definition ######
# add switches
agraph = random_topo.get_small_binomial_graph(10) # arg is seed
...
edges.append(["router1","pc1"])
edges.append(["router1","pc2"])
edges.append(["router5","pc3"])
edges.append(["router5","pc4"])
edges.append(["router2","pc5"])
edges.append(["router4","pc6"])


# make sure that no switches or hosts have the wrong format and add them to the topo
switches = set()
pc_sources = [5,6] # set pcs as sources
for e in edges:
  if "pc" not in e[0]:
    switches.add(e[0])
  else:
