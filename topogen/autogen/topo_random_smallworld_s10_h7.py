...
topoName = "random_smallworld_s10_h7"
...
###### Topology definition ######
# add switches
agraph = random_topo.get_small_smallworld_graph(10) # arg is seed
...
edges.append(["router9","pc2"])
edges.append(["router3","pc3"])
edges.append(["router6","pc4"])
edges.append(["router6","pc5"])
edges.append(["router5","pc6"])
edges.append(["router5","pc7"])

# make sure that no switches or hosts have the wrong format and add them to the topo
switches = set()
pc_sources = [1,2,3] # set pcs as sources
for e in edges:
  if "pc" not in e[0]:
    switches.add(e[0])
  else:
