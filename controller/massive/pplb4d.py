...
version 2.0 - 20201016
Stateful, added self.handled_flow.
Devide config file into global and local for massive experiments


version 1.0 - 20201015
This app - passive PLB for destination pplb4d or destination based path load balancer - will install path flow from redirection point (switch, relay point) to the destination. That means: PPLB4D receives a packet-in event from the sw interested to it containing the source and destination addresses, PPLB then install the path flow from this sw to the destination based on the path it determines for this packet flow. The simplified view of the topo is: S(ource) - SW - D(estination), from S to SW is responsible by the routing app with shortest path first routing algorithm, from SW to D is responsible by the PPLB with different paths for different packet flows.

The routing app has actively discover the destination information for ARPCache: e.g., ARP entry for an IPv4. That feature will be reused in this passive path load balancer for destination (PPLB4D) --> run with routing app.

Use case: This is quite a naive use case, mainly to test the packet-in event contention between different passive control apps, here, including this app, Source-based Passive PLB or PPLB4S, and the Session-based End-point Load Balancer (SBEpLB). It is the reverse of the PPLB4S, any new TCP/UDP session to a server in a given server list will be logged and directed on a new path to the destination, so that the session from the relay point (the switch where the PPLB4D installs its rules) to the destination are balanced/cover more paths as possible.

Use case for Source-based Passive Path Load Balancer or PPLB for Source (PPLB4S): the given servers are broadcasting some live services, which means there may be multiple clients connecting to these servers to download the content, so there may be heavy traffic on the direction from servers to clients, therefore, traffic originating from these servers, upon reaching the balancing points, will be directed on different paths starting from the balancing point if possible to the clients, so that the traffic is distributed on more paths and reduce the possible high load on a particular path.
...
class PPLB4D(arpcache.ARPCache, utility.Utility):

    def __init__(self, *args, **kwargs):
        super(PPLB4D, self).__init__(*args, **kwargs)
        self.logger.info(
            "\tPassive Path Load Balancer for Destination (PPLB4D) flexible, topology aware")
    ...
    def parse_config(self):
        priority, blp = globalconfig.parseGlobalConfig("pplb4d")
        # load local config, contains a list of (ip string,mac string)
        localConfig = globalconfig.localConfigPPLB4d()
    ...
    def pplb4d_packet_in_handler(self, ev):
        ...
        eth_dst = eth.dst
        eth_src = eth.src
        print("\nPPLB4D - packet in dpid: %s, src: %s, dst: %s, in_port: %s" %
        ...
        if eth.ethertype == ether_types.ETH_TYPE_IP:
            ...
            ip_src = ip.src
            ip_dst = ip.dst
            if ((ip_dst, eth_dst) not in self.servers):  # only consider traffic originating from the specified servers
                return
            src_port = None
            dst_port = None
            match = None
