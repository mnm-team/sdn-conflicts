...
[ $# -ne 2 ] && echo -e "please input exactly 2 numbers including the point and router in concern! E.g.: \n bash <thisfilename> <point> <router>" && exit 1

re='^[0-9]+$'
if ! [[ $1 =~ $re && $2 =~ $re ]] ; then
   echo "error: Not a number" >&2; exit 1
fi
configfile=all_config
tmpfile=tmpprocessdumpflow
next=$(($1+1))
grep -n "\<point $1\>" all_config >/dev/null && point=$(grep -n "\<point $1\>" all_config | cut -d: -f1)
[ $? -ne 0 ] && echo "point does not exist, exit" && exit 1
grep -n "\<point $next\>" all_config >/dev/null && npoint=$(grep -n "\<point $next\>" all_config | cut -d: -f1) #npoint = next point
