...
# $5 signifies if this is an isolated run
...
for i in $(seq 1 $4); do echo -n "router$i "; ssh -n router$i "sh -c 'sh $DUMPFLOWS_FILE' > router$i\_dumpflows_$1" ; done
...
for src in $2; do
  for dst in $3; do
    ...
    #for i in $(seq 1 $4); do ssh -n router$i "sh -c 'sed -i \"s/=$tmpportiperf/=56$src$dst/g \" router$i\_dumpflows_$1'"; done
    for i in $(seq 1 $4); do ssh -n router$i "sh -c 'sed -i \"s/=$tmpportnc/=23$src$dst/g;s/=$tmpportiperf/=56$src$dst/g \" router$i\_dumpflows_$1'"; done
