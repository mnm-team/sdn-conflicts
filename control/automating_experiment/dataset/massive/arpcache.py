...
class ARPCache(topology.Topology):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.controller_mac = '00:16:3e:00:00:15'
        self.controller_ip = '192.168.0.15'
	...
	print("arpcache packet in handler")

        eth_dst = eth.dst
        eth_src = eth.src
        dpid = datapath.id
        #print("\npacket in dpid: %s, src: %s, dst: %s, in_port: %s" %(dpid, eth_src, eth_dst, in_port) )
        print("self.arp_cache_db = %s" % self.arp_cache_db)
        ...
        if eth.ethertype == ether_types.ETH_TYPE_ARP:
            ...
            print("arp_msg = "),
            print(arp_msg)
#            print("%s %s %s"%(arp_msg.src_ip, arp_msg.dst_ip, arp_msg.src_mac))
            try:
		    ...
		    pass
    	       #return
                if in_port in self.non_interswitch_ports[dpid] and arp_msg.src_ip not in self.arp_cache_db :#cache it and only cache host's info from switch directly connected to the host.
                    ...
                    print("arp_cache_db = "),
                    print(self.arp_cache_db)
                    #install forwarding rule for the asked switch. Note that, with this approach, only switches who connect directly to end-hosts will send ARP packet-in, the core switches (do not connect to any host) never send ARP packet-in to controller. Unlike normal simple_switch_13.py app where all switches can ask controller things related to ARP, however, this normal app assumes the network topology as a tree, no loop there, which makes it weak. If there is a loop, the line: self.mac_to_port[dpid][src] = in_port will need to update repeatedly and may render this value wrong.
                    actions = [parser.OFPActionOutput(in_port)]
                    match = parser.OFPMatch(eth_type=ether_types.ETH_TYPE_ARP,eth_dst=arp_msg.src_mac)
                    utility.add_flow(self.cookie, datapath, 1, match, actions)
                    #match = parser.OFPMatch(eth_type=ether_types.ETH_TYPE_IP,ip_proto=in_proto.IPPROTO_ICMP ,eth_dst=arp_msg.src_mac)
                    #self.add_flow(self.cookie, datapath, 1, match, actions)
            except KeyError:
                print("ARP_cache: KeyError!")

            if arp_msg.opcode == 1 and arp_msg.dst_ip not in self.arp_cache_db: #do shortcut FLOOD
                print("ARP REQUEST, dst ip not in arp cache")
		if in_port not in self.non_interswitch_ports[dpid]: #install a temporary drop rule on that switch for 5s
                    match = parser.OFPMatch(eth_src=eth_src, arp_tpa=arp_msg.dst_ip, eth_type=ether_types.ETH_TYPE_ARP)
                    action = [] #empty action means drop
                    '''install drop rule for this packet for 5 seconds at the asking switch'''
                    utility.add_flow_with_hard_timeout(self.cookie, datapath, 1, match, action, 5)#timeout = 5s
		    print("install drop rule for ARP_REQUEST at switch %s for 5s"%dpid)
		    return
            ...
            if arp_msg.opcode == 1 and arp_msg.dst_ip in self.arp_cache_db:
                ...
                print("ARP REQUEST, dst ip in arp cache")
                if dpid == self.arp_cache_db[arp_msg.src_ip]['dpid']:#there are cases arp_request are sent from switch other than the responsible switch: since sender sends several arp requests, the first is answered and rules are install on switches on the path, while the subsequent arp requests are based on that installed path and reach the next switch, causing such arp request to be sent from not responsible switch.
                    try:
                        path = nx.shortest_path(self.net,dpid, self.arp_cache_db[arp_msg.dst_ip]['dpid'])
                        print("path from source ip %s to dest ip %s is" % (arp_msg.src_ip, arp_msg.dst_ip)),
                        print(path)
                        #so there is a path from src to dst.
                        #build ARP REPLY : ref:https://sourceforge.net/p/ryu/mailman/message/33076908/
                        arp_reply = packet.Packet()
                        arp_reply.add_protocol(ethernet.ethernet(
                            ethertype=eth.ethertype, dst = eth_src, src = self.arp_cache_db[arp_msg.dst_ip]['mac']))
                        arp_reply.add_protocol(arp.arp(
                            opcode = arp.ARP_REPLY, src_mac = self.arp_cache_db[arp_msg.dst_ip]['mac'], dst_mac=eth_src,
				...
				node_datapath = self.datapathmap[node]
                                ...
                                node_match = parser.OFPMatch(eth_type=ether_types.ETH_TYPE_ARP,eth_src=eth_src, eth_dst=self.arp_cache_db[arp_msg.dst_ip]['mac'])
                                utility.add_flow_with_hard_timeout(self.cookie, node_datapath, 1, node_match, node_action, self.HARD_TIMEOUT)
                                ...
                                #self.add_flow_with_hard_timeout(self.cookie, node_datapath, 1, node_match, node_action, self.HARD_TIMEOUT)
                                print("finish installing rule on node %s" % node)

                        print("install reverse path:")
                        path = nx.shortest_path(self.net, self.arp_cache_db[arp_msg.dst_ip]['dpid'], dpid)
                        print("path from source ip %s to dest ip %s is" % (arp_msg.dst_ip, arp_msg.src_ip)),
                        print(path)
                        #self.add_path_flow(path, eth_dst, eth_src)#remember eth_src = arp_msg.src_mac #from v3.0 and prior, causing possibly the rule with mac address ff:ff:ff:ff:ff:ff in flow table
                        self.add_path_flow(path, self.arp_cache_db[arp_msg.dst_ip]['mac'], eth_src)#remember eth_src = arp_msg.src_mac
                    except nx.NetworkXNoPath:
                        print("There is no path, install a drop rule for 5s to relieve the controller a little bit")
                        match = parser.OFPMatch(eth_src=eth_src, arp_tpa=arp_msg.dst_ip, eth_type=ether_types.ETH_TYPE_ARP)
                        action = [] #empty action means drop
                        '''install drop rule for this packet for 10 seconds at the asking switch'''
                        utility.add_flow_with_hard_timeout(self.cookie, datapath, 1, match, action, 5)#timeout = 5s
	       	else: 
		    #such case occurs when appsuite (this application) works together with other application (like simple_switch_snort) when a message without a matching rule in a switch will be flooded.
                    path = nx.shortest_path(self.net,dpid,self.arp_cache_db[arp_msg.dst_ip]['dpid'])
                    print("asking switch is somewhere else on the network, path from %s to %s is: %s" %(dpid, arp_msg.dst_ip, path))
  	            self.add_path_flow_with_timeout(path, arp_msg.dst_mac, 5) #timeout is 5s
		    return
		    #drop this request since it is from the asking switch that is not related, by installing drop rule for 5s
                    match = parser.OFPMatch(eth_src=eth_src, arp_tpa=arp_msg.dst_ip, eth_type=ether_types.ETH_TYPE_ARP)
                    action = [] #empty action means drop
                    '''install drop rule for this packet for 5 seconds at the asking switch'''
                    utility.add_flow_with_hard_timeout(self.cookie, datapath, 1, match, action, 5)#timeout = 5s
            ...
            if arp_msg.opcode == 2 and arp_msg.dst_ip == self.controller_ip:
                print("dst ip = controller IP, a case for active arpcache discovery!")
                return
                
            if arp_msg.opcode == 2 and arp_msg.dst_ip in self.arp_cache_db:
                #or arp_msg.opcode == arp.ARP_REPLY
                # install a path from src to dst and vice versa, the response will follow the installed path to reach the requested end-point.
                print("ARP RESPONSE")
                try:
		    if arp_msg.src_ip in self.arp_cache_db and arp_msg.dst_ip in self.arp_cache_db:
		        supposed_shortest_path = nx.shortest_path(self.net, self.arp_cache_db[arp_msg.src_ip]['dpid'], self.arp_cache_db[arp_msg.dst_ip]['dpid'])
		        if dpid in supposed_shortest_path:
		            print("path from %s to %s is: %s" %(arp_msg.src_ip, arp_msg.dst_ip, supposed_shortest_path))
		            self.add_path_flow(supposed_shortest_path, arp_msg.src_mac, arp_msg.dst_mac)
                            print("install reverse path:")
                            path = nx.shortest_path(self.net, self.arp_cache_db[arp_msg.dst_ip]['dpid'], self.arp_cache_db[arp_msg.src_ip]['dpid'])
                       	    self.add_path_flow(path, arp_msg.dst_mac, arp_msg.src_mac)
                    	...
                    	path = nx.shortest_path(self.net,dpid,self.arp_cache_db[arp_msg.dst_ip]['dpid'])
                    	print("dpid not in shortest path, path from %s to %s is: %s" %(dpid, arp_msg.dst_ip, path))
  	                self.add_path_flow_with_timeout(path, arp_msg.dst_mac, 5) #timeout is 5s
                except nx.NetworkXNoPath:
            ...
            if arp_msg.opcode == 2 and arp_msg.dst_ip not in self.arp_cache_db:
                ...
                print("ARP RESPONSE! Kernel panic!!!")
                match = parser.OFPMatch(eth_src=eth_src, arp_tpa=arp_msg.dst_ip, eth_type=ether_types.ETH_TYPE_ARP)
                action = [] #empty action means drop
                '''install drop rule for this packet for 5 seconds at the asking switch'''
                utility.add_flow_with_hard_timeout(self.cookie, datapath, 1, match, action, 5)#timeout = 5s
    ...
    def add_path_flow(self, path, src_mac, dst_mac):
		...
		node_datapath = self.datapathmap[node]
                ...
                #node_out = node_parser.OFPPacketOut(datapath=node_datapath, buffer_id=node_proto.OFP_NO_BUFFER, in_port=node_proto.OFPP_CONTROLLER, actions=node_action,data=None)
                #node_datapath.send_msg(node_out)
                node_match = node_parser.OFPMatch(eth_type=ether_types.ETH_TYPE_ARP,eth_src=src_mac, eth_dst=dst_mac)
                ...
                #self.add_flow_with_hard_timeout(self.cookie, node_datapath, 1, node_match, node_action, self.HARD_TIMEOUT) #since path may change, so install rule on the switch with hard_timeout.
    ...
    def add_path_flow_with_timeout(self, path, dst_mac, timeout):
		...
		node_datapath = self.datapathmap[node]
                ...
                #self.add_flow_with_hard_timeout(self.cookie, node_datapath, 1, node_match, node_action, timeout) #since path may change, so install rule on the switch with hard_timeout.



    def discover_arpmapping(self, ip):
        print("ARPCache: actively discover destination ARP mapping using controller info as source!")
